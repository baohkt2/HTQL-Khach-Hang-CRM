...
<div class="container-fluid">
	<div class="row-fluid">
		<div class="span12">
			<h4 style="margin: 10px 0;">Báo cáo thống kê theo dõi liên hệ</h4>
			...
			<form class="form-inline" method="get" action="index.php" style="margin-bottom: 12px;">
				...
				{if $FILTER_FROM neq '' && $FILTER_TO neq ''}
					<a class="btn btn-success" style="margin-left:8px;"
						href="index.php?module=Reports&view=Detail&record={$RECORD_ID}&mode=ExportCSV&from={$FILTER_FROM|escape:'url'}&to={$FILTER_TO|escape:'url'}&user_id={$FILTER_USER_ID|escape:'url'}">
						Export CSV
					</a>
					<a class="btn btn-success" style="margin-left:4px;"
						href="index.php?module=Reports&view=Detail&record={$RECORD_ID}&mode=ExportXLS&from={$FILTER_FROM|escape:'url'}&to={$FILTER_TO|escape:'url'}&user_id={$FILTER_USER_ID|escape:'url'}">
						Export Excel
					</a>
				{/if}
			</form>
