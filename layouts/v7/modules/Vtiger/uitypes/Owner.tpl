...
{if $FIELD_MODEL->get('uitype') eq '53'}
	...
	{if $FIELD_VALUE eq '' && $ASSIGNED_USER_ID neq 'assigned_to_2' && $ASSIGNED_USER_ID neq 'assigned_to_zalo' && $ASSIGNED_USER_ID neq 'assigned_to_facebook'}
	...
	<select class="inputElement select2" type="owner" data-fieldtype="owner" data-fieldname="{$ASSIGNED_USER_ID}" data-name="{$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}" 
            ...
            {/if}
            >
		{if $ASSIGNED_USER_ID eq 'assigned_to_2' || $ASSIGNED_USER_ID eq 'assigned_to_zalo' || $ASSIGNED_USER_ID eq 'assigned_to_facebook' || $FIELD_MODEL->isCustomField() || $VIEW_SOURCE eq 'MASSEDIT'} <option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option> {/if}
