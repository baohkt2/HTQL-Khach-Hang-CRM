  ...
  {},
  {
    ...
    loadFolders: function (folder) {
      app.helper.showProgress(
        app.vtranslate("JSLBL_Loading_Please_Wait") + "..."
      );
      var self = this;
      var params = {
      ...
      settingContainer.find("#serverType").on("change", function (e) {
        ...
        if (serverType == "google-oauth2") {
          ...
          window.location.href =
            "oauth2callback/index.php?authfor=MailManager&authservice=Google";
      ...
      settingContainer.find("#saveMailboxBtn").click(function (e) {
        ...
        if (data["_mbox_server"] == "") {
          vtUtils.showValidationMessage(
            settingContainer.find("#_mbox_server"),
            errorMsg,
            params
          );
          return false;
        ...
        if (data["_mbox_user"] == "") {
          vtUtils.showValidationMessage(
            settingContainer.find("#_mbox_user"),
            errorMsg,
            params
          );
          return false;
        ...
        if (data["_mbox_pwd"] == "") {
          vtUtils.showValidationMessage(
            settingContainer.find("#_mbox_pwd"),
            errorMsg,
            params
          );
          return false;
        ...
        app.helper.showProgress(
          app.vtranslate("JSLBL_Saving_And_Verifying") + "..."
        );
        var params = {
          module: "MailManager",
          view: "Index",
      ...
      container.find("#mmDeleteMail").click(function (e) {
        ...
        if (msgNos.length <= 0) {
          app.helper.showAlertBox({
            message: app.vtranslate("JSLBL_NO_EMAILS_SELECTED"),
          });
          return false;
        } else {
          app.helper
              ...
              app.request.post({ data: params }).then(function (err, data) {
                ...
                if (data.status) {
                  app.helper.showSuccessNotification({
                    message: app.vtranslate("JSLBL_MAILS_DELETED"),
                  });
                  self.updateUnreadCount(
                    "-" + self.getUnreadCountByMsgNos(msgNos),
                    folder
                  );
      ...
      moveToDropDown.on("click", "a", function (e) {
        ...
        if (msgNos.length <= 0) {
          ...
          app.request.post({ data: params }).then(function (err, data) {
            ...
            if (data.status) {
              app.helper.showSuccessNotification({
                message: app.vtranslate("JSLBL_MAIL_MOVED"),
              });
      ...
      container.find("#mmMarkAsUnread").click(function (e) {
        ...
        if (msgNos.length <= 0) {
          ...
          app.request.post({ data: params }).then(function (err, data) {
            ...
            if (data.status) {
              app.helper.showSuccessNotification({
                message: app.vtranslate("JSLBL_MAILS_MARKED_UNREAD"),
              });
      ...
      container.find("#mmMarkAsRead").click(function (e) {
        ...
        if (msgNos.length <= 0) {
          ...
          app.request.post({ data: params }).then(function (err, data) {
            ...
            if (data.status) {
              app.helper.showSuccessNotification({
                message: app.vtranslate("JSLBL_MAILS_MARKED_READ"),
              });
      ...
      container.find("#mm_searchButton").click(function () {
        ...
        if (query.trim() == "") {
          vtUtils.showValidationMessage(
            ...
            app.vtranslate("JSLBL_ENTER_SOME_VALUE")
          );
          return false;
        } else {
    ...
    markMessageUnread: function (msgNos) {
      ...
      if (typeof msgNos == "object") {
        for (var i = 0; i < msgNos.length; i++) {
          ...
          msgEle.data("read", "0");
          var nameSubject =
            "<strong>" + msgEle.find(".nameSubjectHolder").html() + "</strong>";
    ...
    markMessageRead: function (msgNos) {
      ...
      if (typeof msgNos == "object") {
        for (var i = 0; i < msgNos.length; i++) {
          ...
          msgEle.data("read", "1");
          var nameSubject = msgEle
            .find(".nameSubjectHolder")
            .find("strong")
            .html();
          msgEle.find(".nameSubjectHolder").html(nameSubject);
        }
    ...
    getUnreadCountByMsgNos: function (msgNos) {
      ...
      for (var i = 0; i < msgNos.length; i++) {
        var isRead = parseInt(
          container.find("#mmMailEntry_" + msgNos[i]).data("read")
        );
        if (isRead == 0) {
          count++;
        }
    ...
    registerMailCheckBoxClickEvent: function () {
      ...
      container.find(".mailCheckBox").click(function (e) {
        ...
        if (element.is(":checked")) {
          ...
          element
            .closest(".mailEntry")
            .find(".mmDateTimeValue")
            .addClass("mmListDateDivSelected");
        } else {
          ...
          element
            .closest(".mailEntry")
            .find(".mmDateTimeValue")
            .removeClass("mmListDateDivSelected");
    ...
    registerPrevPageClickEvent: function () {
      ...
      container.find("#PreviousPageButton").click(function (e) {
        ...
        self.openFolder(
          folder,
          page,
          jQuery("#mailManagerSearchbox").val(),
    ...
    registerMailClickEvent: function () {
      ...
      container.find(".mmfolderMails").click(function (e) {
        ...
        app.request.post({ data: params }).then(function (err, data) {
          ...
          var nameSubject = jQuery(parentEle)
            .find(".nameSubjectHolder")
            .find("strong")
            .html();
    ...
    loadContentsInIframe: function (element) {
      ...
      element.html(
        '<iframe id="bodyFrame" style="width: 100%; border: none;"></iframe>'
      );
    ...
    registerMailPaginationEvent: function () {
      ...
      container.find(".mailPagination").click(function (e) {
        ...
        app.request.post({ data: params }).then(function (err, data) {
          ...
          var nameSubject = container
            .find("#mmMailEntry_" + msgNo)
            .find(".nameSubjectHolder")
            .find("strong")
            .html();
          container
              ...
              if (data.status) {
                ...
                var previewHtml =
                  '<div class="mmListMainContainer">\n\
										<center><strong>' +
                  app.vtranslate("JSLBL_NO_MAIL_SELECTED_DESC") +
                  "</center></strong></div>";
    ...
    registerForwardEvent: function () {
      ...
      container.find("#mmForward").click(function () {
        ...
        var body = jQuery("#mmBody")
          .find("iframe#bodyFrame")
          .contents()
          .find("html")
          .html();
        ...
        var fwdMsgMetaInfo =
          app.vtranslate("JSLBL_FROM") +
          from +
          "<br/>" +
          app.vtranslate("JSLBL_DATE") +
          date +
          "<br/>" +
          app.vtranslate("JSLBL_SUBJECT") +
          subject;
        if (to != "" && to != null) {
        ...
        var fwdSubject =
          subject.toUpperCase().indexOf("FWD:") == 0
            ? subject
            : "Fwd: " + subject;
        var fwdBody =
          "<p></p><p>" +
          ...
          "</p>" +
          body;
        var attchmentCount = parseInt(
          container.find("#mmAttchmentCount").val()
        );
        if (attchmentCount) {
          var params = {
            module: "MailManager",
          ...
          app.request.post({ data: params }).then(function (err, data) {
            ...
            app.request.post({ data: newParams }).then(function (err, data) {
              ...
              if (err === null) {
                ...
                var descriptionContent = dataObj
                  .find("#iframeDescription")
                  .val();
          ...
          self.openComposeEmailForm("forward", params, {
            subject: fwdSubject,
            body: fwdBody,
          });
        }
    ...
    registerPrintEvent: function () {
      ...
      container.find("#mmPrint").click(function () {
        ...
        content.document.write(
          app.vtranslate("JSLBL_FROM") + " " + from + "<br>"
        );
        content.document.write(app.vtranslate("JSLBL_TO") + " " + to + "<br>");
        if (cc) {
          content.document.write(
            app.vtranslate("JSLBL_CC") + " " + cc + "<br>"
          );
        }
        content.document.write(
    ...
    openReplyEmail: function (all) {
      ...
      var replySubject =
        subject.toUpperCase().indexOf("RE:") == 0 ? subject : "Re: " + subject;
      var replyBody =
        '<p></br></br></p><p style="margin:0;padding:0;">On ' +
        date +
        ", " +
        from +
        ' wrote :</p><blockquote style="border:0;margin:0;border-left:1px solid gray;padding:0 0 0 2px;">' +
        body +
        "</blockquote><br />";
      ...
      if (mailIds) {
        self.openComposeEmailForm("replyall", params, {
          subject: replySubject,
          body: replyBody,
          ids: mailIds,
        });
      } else {
        self.openComposeEmailForm("reply", params, {
          subject: replySubject,
          body: replyBody,
        });
      }
    ...
    showRelatedActions: function () {
      ...
      app.request.post({ data: params }).then(function (err, data) {
        ...
        app.helper.showVerticalScroll(
          container.find("#relationBlock .recordScroll"),
          { autoHideScrollbar: true }
        );
        var iframeHeight =
          jQuery("#mails_container").height() -
          (200 + jQuery("#mailManagerActions").height());
        var contentHeight = jQuery("#bodyFrame")
          .contents()
          .find("html")
          .height();
    ...
    updateUnreadCount: function (count, folder) {
      ...
      if (typeof count == "number") {
        ...
        var oldCount = parseInt(
          container
            .find('.mm_folder[data-foldername="' + folder + '"]')
            .find(".mmUnreadCountBadge")
            .text()
        );
        if (count.substr(0, 1) == "+") {
    ...
    quickCreateSave: function (form, invokeParams) {
      ...
      var params = {
        submitHandler: function (form) {
          ...
          formData = formData
            .replace("module=", "xmodule=")
            .replace("action=", "xaction=");
          if (requestParams) {
          ...
          app.request.post({ data: formData }).then(function (err, data) {
            if (err === null) {
              if (!data.error) {
                ...
                app.helper.hideModal();
                app.helper.showSuccessNotification({
                  message: app.vtranslate("JS_RECORD_CREATED"),
                });
    ...
    saveComment: function (data) {
      ...
      if (commentcontent.trim() == "") {
        ...
        vtUtils.showValidationMessage(
          jQuery('[name="commentcontent"]', data),
          errorMsg,
          validationParams
        );
        return false;
      ...
      app.request.post({ data: params }).then(function (err, response) {
        ...
        if (response.ui) {
          ...
          app.helper.showAlertBox({
            message: app.vtranslate("JSLBL_FAILED_ADDING_COMMENT"),
          });
        }
      });
    },
      ...
      if (element.length > 0) {
        if (element.length == 1) {
          ...
          if (selected) {
            ...
            app.helper.showAlertBox({
              message: app.vtranslate("JSLBL_PLEASE_SELECT_ATLEAST_ONE_RECORD"),
            });
            return false;
          }
        }
      ...
      container.on("change", "#searchType", function (e) {
        ...
        if (element.val() == "ON") {
          ...
          searchBox
            .parent()
            .append(
              '<span class="date-addon input-group-addon"><i class="fa fa-calendar"></i></span>'
            );
