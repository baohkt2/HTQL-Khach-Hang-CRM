...
* All Rights Reserved.
* ****************************************************************************** */
Vtiger.Class("Settings_CTFacebookMessengerIntegration_CTFacebookMessengerIntegration_Js", {}, {
    registerEventForLoginWithFacebook : function () {
        jQuery('#loginWithFacebook').on('click',function () {
            ...
            },{scope: ['public_profile','pages_show_list','pages_messaging','pages_manage_metadata', 'business_management']});
        });
    },
        ...
        $('#saveFacebookPageConfig').live('click', function(){
            ...
            if(pageId != '' && userGroupList != '' && userGroupList != null){
                ...
                AppConnector.request(params).then(function(data) {
                    ...
                    if(recordId != ''){
                        app.helper.showSuccessNotification({message:app.vtranslate('LBL_FACEBOOK_CONFIG_PAGE_UPDATED')});
                    }else{
                        app.helper.showSuccessNotification({message:app.vtranslate('LBL_FACEBOOK_CONFIG_PAGE_ADDED')});
                    }//end of else
                ...
                app.helper.showErrorNotification({message:app.vtranslate('LBL_REQUIRED_FIELD_VALIDATION')});
                return false;
            }
        });//end of function
    },
    ...
    registerEditDeleteFacebookPageConfigEvent: function(){
        ...
        $('.deleteFacebookPageConfig').live('click', function(){
            ...
            app.helper.showConfirmationBox({'message' : deleteMessage}).then(function(data) {
                var params = {
                    'module' : app.getModuleName(),
                    'parent': app.getParentModuleName(),
                    'view' : "CTFacebookChatConfiguration",
    ...
    registerEventsForEnableConfigStatus : function(){
        jQuery('#enableConfigStatusSync').live('switchChange.bootstrapSwitch', function (e) {
            ...
            AppConnector.request(params).then(function(data) {
                if(data){
                    if(currentElement.val() == 'on'){
                        app.helper.showSuccessNotification({message : app.vtranslate('JS_FACEBOOK_CONFIG_STATUS_ENABLED')});
                    }else{
                        app.helper.showSuccessNotification({message : app.vtranslate('JS_FACEBOOK_CONFIG_STATUS_DISABLED')});
                    }
                }
            });            
        });
        ...
        $('#pageId').live('change', function(){
            ...
            AppConnector.request(params).then(function(response) {
                if(response.result.trim() != ''){
                    ...
                    app.helper.showErrorNotification({message: app.vtranslate('LBL_DUPLICATE_FACEBOOK_PAGE')});
                }
            }); 
        });
    },
            ...
            AppConnector.request(params).then(function(data) {
                ...
                if(data.result.facebookPageList){
                    ...
                    $.each(data.result.facebookPageList, function(index,value){
                        $('#pageId').append('<option value="'+index+'">'+value+'</option>');
                    }); 
                    app.helper.showSuccessNotification({message : app.vtranslate('LBL_FACEBOOK_PAGES_SYNCED_SUCCESSFULLY')});
                }else{
                    app.helper.showErrorNotification({message: app.vtranslate('LBL_NO_PAGE_IN_FACEBOOK')});
                }
            });
        });
    },
            ...
            if(flag == 0){
                ...
                AppConnector.request(params).then(function(data) {
                    ...
                    if(data.result.existPageConfig == 0){
                        app.helper.showErrorNotification({message: app.vtranslate('LBL_PLEASE_CONFIGURE_ONE_FACEBOOK_PAGE')});
                    }else if(data.result.existModuleConfig == 0){
                        app.helper.showErrorNotification({message: app.vtranslate('LBL_PLEASE_SELECT_FACEBOOK_MODULE')});
                    }else{
                        window.location.href = 'index.php?parent=Settings&module=CTFacebookMessengerIntegration&view=CTGeneralConfiguration';
                    }
                });            
            }//end of if
        });//end of function
            ...
            AppConnector.request(licenseParams).then(
                function(data) {
                    ...
                    if(data.result == 1){
                        ...
                        text = 'This License key alredy activated for this Domain : ' + currentDomain;
                        app.helper.showErrorNotification({message: text});
                    }else{
...
jQuery(document).ready(function(){
    var instance = new Settings_CTFacebookMessengerIntegration_CTFacebookMessengerIntegration_Js();
