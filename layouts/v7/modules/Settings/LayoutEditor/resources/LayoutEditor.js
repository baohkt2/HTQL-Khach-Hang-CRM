  ...
  {},
  {
    ...
    setRemovedModulesList: function () {
      ...
      if (container.find(".RemovedModulesListArray").length > 0) {
        removedArray = JSON.parse(
          container.find(".RemovedModulesListArray").val(),
        );
      }
    ...
    setInactiveFieldsList: function () {
      ...
      if (contents.find(".inActiveFieldsArray").length > 0) {
        thisInstance.inActiveFieldsList = JSON.parse(
          contents.find(".inActiveFieldsArray").val(),
        );
      }
    },
    /**
    ...
    registerRelatedListEvents: function () {
      ...
      if (container.find(".ModulesListArray").length > 0) {
        allModulesListArray = JSON.parse(
          container.find(".ModulesListArray").val(),
        );
      }
      ...
      selectEle.on("change", function () {
        ...
        if (!selectedOption.is("[data-relation-field-label]")) {
          ...
          liEle
            .find(".deleteRelationShip")
            .attr(
              "data-relation-field-label",
              selectedOption.data("relationFieldLabel"),
            )
            .attr(
              "data-relation-module-label",
              selectedOption.data("relationModuleLabel"),
            );
        }
        liEle
          .data("relationId", selectedVal[0])
          .find(".moduleLabel")
          .text(moduleLabel);
        liEle
          .find(".moduletranslatedLabel")
          .text(selectedOption.data("moduleTranslatedLabel"));
        ulEle.append(
          liEle
            .removeClass("hide moduleCopy")
            .addClass("relatedModule module_" + selectedVal[0]),
        );
        ...
        thisInstance.removeModulesArray.splice(
          thisInstance.removeModulesArray.indexOf(selectedVal[0]),
          1,
        );

        if (!selectEle.find("option").length) {
          jQuery(".hiddenModulesContainer")
            .removeClass("show")
            .addClass("hide");
        }
        thisInstance.showSaveButton();
      ...
      relatedList
        .off("click", ".saveRelatedList")
        .on("click", ".saveRelatedList", function (e) {
          var currentTarget = jQuery(e.currentTarget);
          if (currentTarget.attr("disabled") != "disabled") {
            ...
            for (var key in thisInstance.removeModulesArray) {
              thisInstance.updatedRelatedList["deleted"].push(
                thisInstance.removeModulesArray[key],
              );
            }
    ...
    getUpdatedModulesInfo: function () {
      var thisInstance = this;
      var relatedList = jQuery("#relatedTabOrder");
      var removedModulesList = relatedList
        .find("li.relatedModule")
        .filter(".deleted");
      var updatedModulesList = relatedList
        .find("li.relatedModule")
        .not(".deleted");
      ...
      if (saveButton.css("opacity") == "0") {
        ...
        app.helper.showAlertNotification({
          message: app.vtranslate(
            "JS_SAVE_THE_CHANGES_TO_UPDATE_FIELD_SEQUENCE",
          ),
        });
      }
    },
    ...
    createUpdatedBlocksList: function (currentField) {
      ...
      if (
        jQuery.inArray(updatedBlockId, thisInstance.updatedBlocksList) == -1
      ) {
    ...
    registerHeaderSummaryDependency: function (container, fieldContainer) {
      var thisInstance = this;
      container
        .find('input[name="headerfield"], input[name="summaryfield"]')
        .on("change", function (e) {
          ...
          if (currentElement.is(":checked")) {
            container
              .find('input[type="checkbox"][name="' + dependentFlagName + '"]')
              .prop("checked", false)
              .attr("readonly", "readonly")
              .removeClass("cursorPointer")
              .addClass("cursorPointerNotAllowed");
          } else {
            if (dependentFlagName === "headerfield") {
              if (thisInstance.nameFields.indexOf(fieldName) === -1) {
                container
                  .find(
                    'input[type="checkbox"][name="' + dependentFlagName + '"]',
                  )
                  .removeAttr("readonly", "readonly")
                  .removeClass("cursorPointerNotAllowed")
    ...
    showFieldEditModel: function (data, blockId, fieldContainer) {
      ...
      data.find('input[name="pickListValues"]').on("change", function (e) {
        ...
        var defaultOptions = defaultField
          .find("option")
          .removeClass("doestNotExists")
          .addClass("doesNotExists");
        var emptyOption = defaultOptions
          .filter('[value=" "]')
          .removeClass("doesNotExists");
        ...
        for (var i in fieldOptions) {
          ...
          var fieldValueOption = defaultOptions.filter(
            '[value="' + fieldValue + '"]',
          );
      ...
      data.find('[name="defaultvalue"]').on("change", function (e) {
        var currentTarget = jQuery(e.currentTarget);
        var defaultValueUi = currentTarget
          .closest("span")
          .find(".defaultValueUi");
      ...
      data.find('[name="presence"]').on("change", function (e) {
        var currentTarget = jQuery(e.currentTarget);
        if (currentTarget.attr("readonly") == "readonly") return;
        if (currentTarget.is(":checked")) {
      ...
      // Toggle age select khi admin check/uncheck ràng buộc tuổi
      data.find(".ageLimitCheckbox").on("change", function () {
        var ageLimitSelect = data.find(".ageLimitSelect");
        if (jQuery(this).is(":checked")) {
          ageLimitSelect.removeClass("hide");
        } else {
          ageLimitSelect.addClass("hide").val("");
        }
      });
        ...
        submitHandler: function (form) {
          ...
          if (!isEditMode) {
            if (fieldTypeValue == "Decimal") {
              ...
              if (
                decimalFieldLengthValue > fieldLength &&
                !(fieldLength < 0) &&
                fieldLength >= 59
              ) {
                var message =
                  app.vtranslate("JS_LENGTH_SHOULD_BE_LESS_THAN_EQUAL_TO") +
                  " " +
                  fieldLength;
            ...
            if (
              ...
              var select2Element =
                app.getSelect2ElementFromSelect(pickListValueElement);
              ...
              var lowerCasedpickListValuesArray = jQuery.map(
                pickListValuesArray,
                function (item, index) {
                  return item.toLowerCase();
                },
              );
              var uniqueLowerCasedpickListValuesArray = jQuery.unique(
                lowerCasedpickListValuesArray,
              );
              var uniqueLowerCasedpickListValuesArraySize =
                uniqueLowerCasedpickListValuesArray.length;
              var arrayDiffSize =
                pickListValuesArraySize -
          ...
          if (isEditMode) {
            ...
            if (
              typeof saveParams["fieldDefaultValue[]"] == "undefined" &&
              typeof saveParams["fieldDefaultValue"] == "undefined"
            ) {
              saveParams["fieldDefaultValue"] = "";
            }
            thisInstance.saveFieldDetails(saveParams).then(
              function (result) {
                ...
                if (response.presence) {
                  ...
                  if (
                    ...
                    thisInstance.inActiveFieldsList[blockId] = {};
                    thisInstance.inActiveFieldsList[blockId][fieldId] =
                      response["label"];
                  } else {
                    thisInstance.inActiveFieldsList[blockId][fieldId] =
                      response["label"];
              ...
              function () {
                ...
                app.helper.showAlertNotification({
                  message: app.vtranslate(
                    ...
                    thisInstance.maxNumberOfHeaderFields,
                  ),
                });
              },
            );
              ...
              function (data) {
                ...
                if (data) {
                  ...
                  var customFieldsCount =
                    blockElement.data("customFieldsCount");
    ...
    registerAddCustomFieldEvent: function () {
      ...
      contents.find(".addCustomField").click(function (e, params) {
        ...
        var addFieldContainer = contents
          .find(".createFieldModal")
          .clone(true, true);
    ...
    addCustomField: function (blockId, form) {
      ...
      if (params["decimal"]) params["decimal"] = parseInt(params["decimal"]);
    ...
    registerFieldTypeChangeEvent: function (form, nameAttrsList) {
      ...
      form.find('[name="fieldType"]').on("change", function (e) {
        ...
        if (typeof maxlengthValue === "undefined") maxlengthValue = "255";
        ...
        // Phone: hiển thị trường giới hạn độ dài số điện thoại
        if (selectedOption.data("phonelimitsupported")) {
          form.find(".phoneLimitSupported").removeClass("hide");
        }

        // Date: hiển thị ràng buộc tuổi
        if (selectedOption.data("agelimitsupported")) {
          form.find(".ageLimitSupported").removeClass("hide");
        }
        ...
        if (
          form
            .find('input[type="checkbox"][name="defaultvalue"]')
            .data("defaultDisabled") == "1"
        ) {
          form
            .find('input[type="checkbox"][name="defaultvalue"]')
            .attr("readonly", "readonly")
        ...
        for (var i in nameAttrsList) {
          ...
          var defaultValueUiContainer =
            defaultValueUi.closest(".defaultValueUi");
          ...
          defaultValueUiContainer
            .find("[data-rule-required]")
    ...
    showCustomField: function (result) {
      ...
      fieldContainer
        ...
        .attr("data-field-name", result["name"]);
      fieldCopy
        .find(".deleteCustomField, .saveFieldDetails, .fieldProperties")
        .attr("data-field-id", result["id"]);
      ...
      if (firstSortableChildren.filter("li.dummyRow").length > 0) {
        firstSortableChildren
          .filter("li.dummyRow")
          .detach()
          .appendTo(sortable2);
      } else {
        secondSortableChildren
          .filter("li.dummyRow")
          .detach()
          .appendTo(sortable1);
      }
    ...
    setFieldDetails: function (result, form) {
      ...
      if (!result["mandatory"]) {
        ...
        mTitle = app.vtranslate(
          "JS_MAKE_THIS_FIELD",
          app.vtranslate("JS_PROP_MANDATORY"),
        );
      } else {
        form.find(".mandatory").addClass("cursorPointer");
      ...
      if (!result["quickcreate"]) {
        ...
        qTitle = app.vtranslate(
          "JS_SHOW_THIS_FIELD_IN",
          app.vtranslate("JS_QUICK_CREATE"),
        );
      } else {
        form.find(".quickCreate").removeClass("disabled");
        qTitle = app.vtranslate(
          "JS_HIDE_THIS_FIELD_IN",
          app.vtranslate("JS_QUICK_CREATE"),
        );
      }
      ...
      if (!result["isSummaryField"]) {
        ...
        sTitle = app.vtranslate(
          "JS_SHOW_THIS_FIELD_IN",
          app.vtranslate("JS_KEY_FIELD"),
        );
      } else {
      ...
      if (!result["isHeaderField"]) {
        ...
        hTitle = app.vtranslate(
          "JS_SHOW_THIS_FIELD_IN",
          app.vtranslate("JS_DETAIL_HEADER"),
        );
      } else {
      ...
      if (!result["masseditable"]) {
        ...
        meTitle = app.vtranslate(
          "JS_SHOW_THIS_FIELD_IN",
          app.vtranslate("JS_MASS_EDIT"),
        );
      } else {
      ...
      if (!result["fieldDefaultValue"]) {
        ...
        html +=
          "<span>" + app.vtranslate("JS_DEFAULT_VALUE_NOT_SET") + "</span>";
        html += "</div>";
        ...
        if (isObject && typeof defaultValue == "object") {
          ...
          jQuery.each(defaultValue, function (fieldName, value) {
            ...
            if (value) {
              html +=
                '<span data-defaultvalue-fieldname="' +
                fieldName +
                '" data-defaultvalue="' +
                value +
                '">' +
                app.vtranslate("JS_DEFAULT_VALUE") +
                app.vtranslate("JS_" + fieldName) +
                " : </span><span>" +
                value +
                "</span>";
            } else {
              html +=
                "<span>" +
                app.vtranslate("JS_DEFAULT_VALUE_NOT_SET") +
                "</span>";
            }
            html += "</div>";
          });
    ...
    registerAddCustomBlockEvent: function () {
      ...
      contents.find(".addCustomBlock").click(function (e) {
        var addBlockContainer = contents
          .find(".addBlockModal")
          .clone(true, true);
        ...
        var callBackFunction = function (data) {
          ...
          vtUtils.showSelect2ElementView(
            data.find("select").addClass("select2"),
          );
          ...
          var params = {
            submitHandler: function (form) {
              ...
              if (
                jQuery.inArray(blockLabelValue, thisInstance.blockNamesList) ==
                -1
              ) {
                thisInstance.saveBlockDetails(form).then(function (data) {
                  var params = {};
                  if (data) {
                    var result = data;
                    thisInstance.displayNewCustomBlock(result);
                    thisInstance.updateNewSequenceForBlocks(
                      result["sequenceList"],
                    );
    ...
    displayNewCustomBlock: function (result) {
      ...
      var newBlockCloneCopy = contents
        .find(".newCustomBlockCopy")
        .clone(true, true);
      newBlockCloneCopy
        .attr("data-block-id", result["id"])
        .find(".blockLabel")
        .append(jQuery("<strong>" + result["label"] + "</strong>"));
      newBlockCloneCopy
        .find(".blockVisibility")
        .attr("data-block-id", result["id"]);
      beforeBlock.after(
        newBlockCloneCopy
          .removeClass("hide newCustomBlockCopy")
          .addClass("editFieldsTable block_" + result["id"])
          .attr("id", "block_" + result["id"]),
      );
      newBlockCloneCopy
        .find("#hiddenCollapseBlock")
        .addClass("bootstrap-switch");
      newBlockCloneCopy
        .find("#hiddenCollapseBlock")
        .attr("name", "collapseBlock");
      ...
      newBlockCloneCopy
        .find(".blockFieldsList")
        .sortable({ connectWith: ".blockFieldsList" });
    },
    /**
    ...
    registerInactiveFieldsEvent: function () {
      ...
      contents.on("click", "button.inActiveFields", function (e) {
        ...
        if (jQuery.isEmptyObject(thisInstance.inActiveFieldsList[blockId])) {
          ...
          var inActiveFieldsContainer = contents
    ...
    showHiddenFields: function (blockId, form) {
      ...
      jQuery.each(
        ...
        function (key, value) {
          if (fieldCount % 3 === 0) {
            curRow = $('<div class="row"></div>').appendTo(
              form.find(".inActiveList .list"),
            );
            fieldCount = 0;
          }
          var inActiveField = jQuery(
            '<div class="col-sm-4">\n\
											<div class="checkbox">\n\
												<label><input type="checkbox" class="inActiveField" value="' +
              key +
    ...
    registerDeleteCustomFieldEvent: function (contents) {
      ...
      contents.find("a.deleteCustomField").click(function (e) {
        ...
        if (currentTarget.data("oneOneRelationship") == "1") {
          message = app.vtranslate(
            "JS_ONE_ONE_RELATION_FIELD_DELETE",
            currentTarget.data("currentFieldLabel"),
            currentTarget.data("currentModuleLabel"),
            currentTarget.data("relationFieldLabel"),
            currentTarget.data("relationModuleLabel"),
          );
        } else if (currentTarget.data("relationshipField") == "1") {
          message = app.vtranslate(
            "JS_TAB_FIELD_DELETION",
            currentTarget.data("relationFieldLabel"),
            currentTarget.data("relationModuleLabel"),
            currentTarget.data("currentTabLabel"),
            currentTarget.data("currentModuleLabel"),
          );
        }
    ...
    saveFieldDetails: function (params) {
      ...
      var fieldId = params["fieldid"];
      if (
        !this.isHeaderAllowed() &&
        params.headerfield == true &&
        this.headerFieldsMeta[fieldId] !== 1
      ) {
        aDeferred.reject();
      } else {
        this.updateHeaderFieldMeta(params);
    ...
    registerModulesChangeEvent: function () {
      ...
      vtUtils.showSelect2ElementView(
        container.find('[name="layoutEditorModules"]'),
      );
      ...
      container.on("change", '[name="layoutEditorModules"]', function (e) {
        ...
        thisInstance
          .getModuleLayoutEditor(selectedModule)
          .then(function (data) {
    ...
    registerEventForCollapseBlock: function () {
      jQuery("#moduleBlocks").on(
        "switchChange.bootstrapSwitch",
        "input[name='collapseBlock']",
        function (e) {
          var currentElement = jQuery(e.currentTarget);
    ...
    registerSwitchActionOnFieldProperties: function () {
      ...
      contents.on("click", ".fieldProperties .switch", function (e) {
        var element = jQuery(e.currentTarget);
        if (
          typeof element.find("i").attr("readonly") != "undefined" ||
        ...
        if (fieldPropertyName === "headerfield") {
          ...
          if (thisInstance.nameFields.indexOf(fieldName) !== -1) {
            app.helper.showAlertNotification({
              message: app.vtranslate(
                "JS_NAME_FIELDS_APPEAR_IN_HEADER_BY_DEFAULT",
              ),
            });
            return;
          }
        ...
        if (
          fieldPropertyName === "summaryfield" ||
          fieldPropertyName === "headerfield"
        ) {
          //Field can either be summary or header enabled
          var dependentFieldPropertySelector =
            fieldPropertyName === "summaryfield" ? ".header" : ".summary";
          var dependentPropertyElement = jQuery(
            dependentFieldPropertySelector,
            fieldPropertiesHolder,
          );
          if (!dependentPropertyElement.hasClass("disabled")) {
            app.helper.showAlertNotification({
              message: app.vtranslate(
                "JS_FIELD_CAN_EITHER_BE_HEADER_OR_SUMMARY_ENABLED",
              ),
            });
            return;
          }
    ...
    registerAddCustomField: function () {
      jQuery(".dummyRow .addButton").on("click", function (e) {
        ...
        var addCustomFieldBtn = currentDummyRow
    ...
    disableOrEnableRealtionFieldTabLabels: function (container) {
      var relationTypeEleSelected = container.find(
        ".relationImages img.selected",
      );
      var fieldsToEnable = relationTypeEleSelected.data(
        "supportedFieldToEnable",
      );
      ...
      for (var index in fieldsToEnable) {
        ...
        if (
          (relModule == "Calendar" || relModule == "Documents") &&
    ...
    triggerFieldListTabClickEvent: function () {
      ...
      contents.find(".detailViewTab").click(function (e) {
        ...
        thisInstance
          .showFieldsListUI(detailViewLayout, e)
          .then(function (data) {
    ...
    showFieldsListUI: function (detailViewLayout, e) {
      ...
      var url =
        selectedTab.data("url") +
        "&sourceModule=" +
        jQuery("#selectedModuleName").val() +
        "&mode=" +
        mode;
      jQuery(".selectedMode").val(mode);
    ...
    triggerDuplicationTabClickEvent: function () {
      ...
      contents.find(".duplicationTab").click(function (e) {
        ...
        thisInstance
          .showDuplicationHandlingUI(duplicationContainer, e)
          .then(function (data) {
            var form = jQuery(".duplicateHandlingForm");
            var duplicateHandlingContainer = form.find(
            ...
            if (dupliCheckEle.length > 0) {
              ...
              dupliCheckEle
                .bootstrapSwitch("handleWidth", "43px")
                .bootstrapSwitch("labelWidth", "43px")
                .bootstrapSwitch("size", "86px");
            }
            ...
            var params = {
              submitHandler: function (form) {
                ...
                var params = form.serializeFormData();
                if (
                  typeof params["fieldIdsList[]"] == "undefined" &&
                  typeof params["fieldIdsList"] == "undefined"
                ) {
                  params["fieldIdsList"] = "";
                }
                ...
                app.request.post({ data: params }).then(function (error, data) {
                  ...
                  if (error == null) {
                    var message = app.vtranslate(
                      "JS_DUPLICATE_HANDLING_SUCCESS_MESSAGE",
                    );
                    if (params.rule == 1) {
                    ...
                    app.helper.showErrorNotification({
                      message: app.vtranslate(
                        "JS_DUPLICATE_HANDLING_FAILURE_MESSAGE",
                      ),
                    });
                  }
                });
    ...
    showDuplicationHandlingUI: function (duplicationContainer, e) {
      var aDeferred = jQuery.Deferred();
      var duplicateUiContainer = duplicationContainer.find(
        ".duplicateHandlingDiv",
      );
...
Settings_LayoutEditor_Js(
  "Settings_LayoutEditor_Index_Js",
  {},
  {
    init: function () {
      this.addComponents();
