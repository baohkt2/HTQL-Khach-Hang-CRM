...
class PDFMaker_PDFMaker_Model extends Vtiger_Module_Model {
    ...
    public function GetListviewData() {
        $sql = "SELECT vtiger_pdfmaker.templateid, vtiger_pdfmaker.filename, vtiger_pdfmaker.description, vtiger_pdfmaker.module
                FROM vtiger_pdfmaker LEFT JOIN vtiger_pdfmaker_settings USING(templateid) WHERE vtiger_pdfmaker.deleted = 0 ORDER BY vtiger_pdfmaker.templateid ASC";
        ...
        for ($i = 0; $i < $num_rows; $i++) {
            ...
            $pdftemplatearray['filename'] = $this->db->query_result($result, $i, 'filename');
            ...
            $pdftemplatearray['module_name'] = $currModule;
            $pdftemplatearray['edit'] = "<li><a href=\"index.php?module=PDFMaker&view=EditFree&return_view=List&templateid=" . $templateid . "\">" . vtranslate("LBL_EDIT", 'PDFMaker') . "</a></li>";
            $pdftemplatearray['delete'] = "<li><a href=\"javascript:void(0)\" class=\"pdfmakerDeleteTemplate\" data-id=\"" . $templateid . "\">" . vtranslate("LBL_DELETE", 'PDFMaker') . "</a></li>";
     ...
     */
    public function GetAllEntityModules() {
        $sql = "SELECT name, tablabel FROM vtiger_tab WHERE presence IN (0,2) AND isentitytype = 1 ORDER BY tablabel ASC";
        $result = $this->db->pquery($sql, array());
        $modules = array();
        while ($row = $this->db->fetchByAssoc($result)) {
            $modules[] = array('name' => $row['name'], 'label' => vtranslate($row['name'], $row['name']));
        }
        return $modules;
    }
    ...
    public function GetPreparedMPDF(&$mpdf, $record, $module, $language, $templateid = null) {
        ...
        $PDFContent = $this->GetPDFContentRef($module, $focus, $language, $templateid);

        if (!empty($templateid)) {
            $Settings = $PDFContent->getSettings($templateid);
        } else {
    ...
    public function GenerateName($record, $module) {
        ...
        if (isset($focus->column_fields[$fieldname]) && $focus->column_fields[$fieldname] != "") {
            ...
            $name = $module . "_" . $record . "_" . date("ymdHi");
        }
        return $name;
    }

    public function GetPDFContentRef($module, $focus, $language, $templateid = null) {
        return new PDFMaker_PDFContent_Model($module, $focus, $language, $templateid);
    }
    ...
    function generate_cool_uri($name) {
        ...
        if (function_exists('transliterator_transliterate')) {
            $return = transliterator_transliterate('Any-Latin; Latin-ASCII; [\u0080-\u7fff] remove', $return);
        }
